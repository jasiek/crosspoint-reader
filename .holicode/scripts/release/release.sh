...
# Logging compatibility — common.sh exports info/warn/err
log_info() { info "$@"; }
log_warn() { warn "$@"; }
...
generate_changelog() {
    ...
    local changelog_version=${next_version#v}

    log_info "Generating changelog for ${next_version} since ${last_tag}..."

    # Build the release section (bare SemVer per Keep a Changelog convention)
    echo "## [${changelog_version}] - $(date +%Y-%m-%d)" > "${temp_changelog_file}"
    ...
    breaking_changes=$(git log "${last_tag}"..HEAD --grep="BREAKING CHANGE" --pretty=format:"- %B" | grep -A 10 "BREAKING CHANGE" || true)
    if [ -n "$breaking_changes" ]; then
        log_warn "Breaking changes detected. Adding to changelog."
        echo "### BREAKING CHANGES" >> "${temp_changelog_file}"
        echo "$breaking_changes" >> "${temp_changelog_file}"
        echo -e "\n" >> "${temp_changelog_file}"
    fi

    if [ -f "$changelog_file" ]; then
        # Insert the new section after ## [Unreleased] in the existing file
        local insert_file=$(mktemp)
        local found_unreleased=false
        while IFS= read -r line; do
            echo "$line" >> "$insert_file"
            if [[ "$found_unreleased" == false && "$line" =~ ^##[[:space:]]*\[Unreleased\] ]]; then
                found_unreleased=true
                echo "" >> "$insert_file"
                cat "${temp_changelog_file}" >> "$insert_file"
            fi
        done < "$changelog_file"
        if [[ "$found_unreleased" == false ]]; then
            # No Unreleased header found — prepend section after H1
            local prepend_file=$(mktemp)
            head -1 "$changelog_file" > "$prepend_file"
            echo "" >> "$prepend_file"
            cat "${temp_changelog_file}" >> "$prepend_file"
            tail -n +2 "$changelog_file" >> "$prepend_file"
            mv "$prepend_file" "$changelog_file"
        else
            mv "$insert_file" "$changelog_file"
        fi
        rm -f "$insert_file"
    else
        # No existing changelog — create full Keep a Changelog skeleton
        {
            echo "# Changelog"
            echo ""
            echo "All notable changes to the HoliCode Framework are documented in this file."
            echo ""
...
update_version_files() {
    ...
    if [ -f "package.json" ]; then
        sed_i "package.json" "s/\"version\": \"[^\"]*\"/\"version\": \"${version_number}\"/"
    ...
    if [ -f ".holicode/VERSION" ]; then
        echo "${version_number}" > .holicode/VERSION
        log_info "Updated .holicode/VERSION"
    fi

    # Update mb_meta.version in all HoliCode state files
    for state_file in .holicode/state/*.md; do
        [ -f "$state_file" ] || continue
        if grep -q 'mb_meta:' "$state_file" 2>/dev/null; then
            sed_i "$state_file" "s/  version: \"[^\"]*\"/  version: \"${version_number}\"/"
            log_info "Updated ${state_file}"
        fi
    done

    # Update .clinerules/holicode.md header version
    if [ -f ".clinerules/holicode.md" ]; then
        sed_i ".clinerules/holicode.md" "s/^# HoliCode Framework v[^[:space:]]*/# HoliCode Framework v${version_number}/"
        log_info "Updated .clinerules/holicode.md"
    fi
...
commit_release_changes() {
    ...
    git add .holicode/VERSION 2>/dev/null || true
    git add .holicode/state/*.md 2>/dev/null || true
    git add .clinerules/holicode.md 2>/dev/null || true
...
generate_release_notes() {
    ...
    local changelog_version=${version#v}

    log_info "Generating release notes for GitHub for ${version}..."

    # Extract relevant section from changelog (bare SemVer headings)
    local changelog_section=$(sed -n "/^## \[${changelog_version}\]/,/^## \[/p" CHANGELOG.md | sed '$d' | tail -n +2)
